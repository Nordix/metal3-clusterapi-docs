...
Building the plugin is as simple as `make`.
...
See all [Makefile targets](#make-targets).
...
1. `all` (default): `build` - builds the binary
1. `install`: builds and install the binary
1. `registry` run Docker registry and pull `busybox` image for testing
1. `certificates` generate test certificates into `examples` directory
1. `test` install the plugin, load test image, generate certificates and
   and sign the test image using the `rsassa-pss-sha512.sh`
1. `inspect` checks the signatures and structure in the repository
1. `verify` runs `test`, `inspect` and then verifies the signature
1. `clean` deletes all of the resources creates, stops Docker registry etc
