...
sudo su -l -c "minikube ssh sudo brctl addbr ironicendpoint2" "${USER}"
sudo su -l -c "minikube ssh sudo  ip link set ironicendpoint2 up" "${USER}"
sudo su -l -c "minikube ssh sudo  brctl addif  ironicendpoint2 eth3" "${USER}"
sudo su -l -c "minikube ssh sudo  ip addr add 172.23.0.2/24 dev ironicendpoint2" "${USER}"
# Firewall
